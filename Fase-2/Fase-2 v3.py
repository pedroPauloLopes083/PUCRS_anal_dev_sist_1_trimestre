...
def visualizar_dados(*args):
  dados, inicio_mes, inicio_ano, fim_mes, fim_ano, tipo = args
  ...
  while True:
    ...
    inicio_mes = input("Digite o mês inicial (MM): ")
    if not inicio_mes.isdigit() or len(inicio_mes) != 2:
      print("Mês inválido. Digite um valor numérico com dois dígitos (ex: 01).")
      continue
    ...
    try:
      ...
      print("Ano inválido. Digite um valor numérico com quatro dígitos (ex: 2006).")
      continue

    # Validação do mês final
    fim_mes = input("Digite o mês final (MM): ")
    if not fim_mes.isdigit() or len(fim_mes) != 2:
      print("Mês inválido. Digite um valor numérico com dois dígitos (ex: 01).")
      continue

    # Validação do ano final
    try:
      fim_ano = int(input("Digite o ano final (AAAA): "))
    except ValueError:
      print("Ano inválido. Digite um valor numérico com quatro dígitos (ex: 2006).")
      continue
    ...
    if tipo not in ["todos", "precipitacao", "temperatura", "umidade_vento"]:
      print("Tipo de dado inválido. Digite um dos tipos válidos: todos, precipitacao, temperatura, umidade_vento.")
      continue
